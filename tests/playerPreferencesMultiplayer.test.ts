...
import { clonePlayerPreferences, loadPlayerPreferences, savePlayerPreferences, withGameSpeed, PLAYER_SPEED_DEFAULT_MIGRATION_KEY, type PreferenceStorage } from "@/lib/preferences/playerPreferences";
...
describe("multiplayer preference isolation", () => {
  it("does not send preferences or identity while creating a room", async () => { let body = ""; vi.stubGlobal("fetch", vi.fn(async (_url, init?: RequestInit) => { body = String(init?.body); return new Response(JSON.stringify({ data: {} }), { status: 200, headers: { "Content-Type": "application/json" } }); })); await createMultiplayerRoom({ rules: { presetId: "contree-kffr" } }, { access_token: "token" }); expect(body).not.toContain("preferences"); expect(body).not.toContain("displayName"); expect(body).toContain("rules"); });
  it("lets two clients retain different personal preferences", () => { const antonin = new MemoryStorage(); const ben = new MemoryStorage(); const first = clonePlayerPreferences(); first.cards.cardSize = "large"; first.visual.tableTheme = "midnight-blue"; first.visual.theme = "light"; const second = clonePlayerPreferences(); second.cards.cardSize = "small"; second.visual.theme = "dark"; savePlayerPreferences(antonin, first); savePlayerPreferences(ben, second); expect(loadPlayerPreferences(antonin).cards.cardSize).toBe("large"); expect(loadPlayerPreferences(ben).cards.cardSize).toBe("small"); expect(loadPlayerPreferences(antonin).visual.tableTheme).toBe("midnight-blue"); expect(loadPlayerPreferences(ben).visual.tableTheme).not.toBe("midnight-blue"); expect(loadPlayerPreferences(antonin).visual.theme).toBe("light"); expect(loadPlayerPreferences(ben).visual.theme).toBe("dark"); });
  it("keeps a solo speed change entirely inside local storage", () => { const storage = new MemoryStorage(); const beforeKeys = [...storage.values.keys()]; savePlayerPreferences(storage, withGameSpeed(clonePlayerPreferences(), "fast")); expect(beforeKeys).toEqual([]); expect([...storage.values.keys()]).toEqual(["coinche:player-preferences:v1", PLAYER_SPEED_DEFAULT_MIGRATION_KEY]); expect(loadPlayerPreferences(storage).gameplay.gameSpeed).toBe("fast"); });
