...
describe("Bot Lab Monte Carlo V3", () => {
  it("1. partenaire maître : conserve As et 10", () => {
    ...
    expect(chooseMonteCarloV3Decision(state).card).toEqual(c("7", "clubs"));
  });
  ...
  it("2. gagner au minimum : prend avec le 10 plutôt que l'As", () => {
    ...
    expect(chooseMonteCarloV3Decision(state).card).toEqual(c("10", "clubs"));
  });
  ...
  it("4. adversaire coupe : évite l'As vulnérable", () => {
    ...
    expect(chooseMonteCarloV3Decision(state).card).not.toEqual(c("A", "clubs"));
  });
  ...
  it("5. partenaire coupe : ouvre la couleur de coupe", () => {
    ...
    expect(chooseMonteCarloV3Decision(state).card.suit).toBe("clubs");
  });
  ...
  it("6. atout fort : le petit atout suffit", () => {
    ...
    expect(chooseMonteCarloV3Decision(state).card).toEqual(c("8", "spades"));
  });
  ...
  it("8. tirage atout : tire avec contrôle clair", () => {
    ...
    expect(chooseMonteCarloV3Decision(state).card.suit).toBe("spades");
  });
  ...
  it("9. ne pas tirer atout : conserve la coupe utile du partenaire", () => {
    ...
    expect(chooseMonteCarloV3Decision(state).card.suit).not.toBe("spades");
  });
  ...
  it("respecte les contraintes fortes de couleur dans toutes les distributions produites", () => {
    ...
    for (let seed = 1; seed <= 10; seed += 1) {
      ...
      if (plausible) expect(plausible.hands[1].some((card) => card.suit === "clubs")).toBe(false);
    }
  });
