...
export function FriendsPageClient() {
  ...
  const refreshSnapshot = useCallback(async (activeSession: Session | null) => {
    ...
    try {
      const nextSnapshot = await fetchSocialSnapshot(activeSession);
  ...
  async function runMutation(
    key: string,
    operation: (activeSession: Session) => Promise<SocialMutationResult>,
    successMessage: string | ((result: SocialMutationResult) => string),
  ) {
