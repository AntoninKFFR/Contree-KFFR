...
export function AccessibleDialog({ children, closeLabel, description, footer, minimalHeader = false, onClose, showCloseButton = true, stableHeight = false, title, width = "wide" }: Props) {
  ...
  return <div aria-describedby={description ? descriptionId : undefined} aria-labelledby={titleId} aria-modal="true" className="fixed inset-0 z-[60] flex items-center justify-center bg-[#020704]/80 p-0 backdrop-blur-sm sm:p-3" onClick={(event) => { if (event.target === event.currentTarget) onClose(); }} role="dialog">
    <section className={`coinche-dialog flex h-[100dvh] max-h-[100dvh] w-full flex-col overflow-hidden border border-white/10 bg-[#e9e4d8] shadow-[0_30px_100px_rgb(0_0_0_/_55%)] sm:max-h-[calc(100dvh-1.5rem)] sm:rounded-3xl ${stableHeight ? "sm:h-[min(46rem,calc(100dvh-1.5rem))]" : "sm:h-auto"} ${width === "wide" ? "sm:max-w-6xl" : "sm:max-w-3xl"}`} onClick={(event) => event.stopPropagation()} ref={panelRef}>
      <header className={`sticky top-0 z-30 flex shrink-0 items-center justify-between gap-4 border-b border-white/10 bg-[#0c1c15] px-4 text-stone-100 backdrop-blur sm:px-6 ${minimalHeader ? "py-3" : "py-3.5"}`}><div className="min-w-0">{minimalHeader ? null : <p className="text-[9px] font-black uppercase tracking-[0.18em] text-emerald-300/60">Contrée KFFR</p>}<h2 className={`${minimalHeader ? "text-lg" : "mt-0.5 text-xl"} font-black text-[#f4ead0]`} id={titleId}>{title}</h2>{description ? <p className="mt-0.5 text-sm text-white/[0.48]" id={descriptionId}>{description}</p> : null}</div>{showCloseButton ? <button aria-label={closeLabel ?? `Fermer ${title}`} className="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border border-white/10 bg-white/[0.045] text-white/65 transition hover:border-white/20 hover:bg-white/[0.1] hover:text-white focus-visible:outline focus-visible:outline-3 focus-visible:outline-offset-2 focus-visible:outline-emerald-300" onClick={onClose} ref={closeRef} type="button"><svg aria-hidden="true" className="h-4 w-4" fill="none" viewBox="0 0 24 24"><path d="M6 6l12 12M18 6 6 18" stroke="currentColor" strokeLinecap="round" strokeWidth="1.8" /></svg></button> : null}</header>
