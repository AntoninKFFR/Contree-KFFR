...
export function AccessibleDialog({ children, backdropClassName = "", closeLabel, description, footer, minimalHeader = false, onClose, showCloseButton = true, stableHeight = false, title, width = "wide" }: Props) {
  ...
  return <div aria-describedby={description ? descriptionId : undefined} aria-labelledby={titleId} aria-modal="true" className={`coinche-dialog-backdrop fixed inset-0 z-[60] flex items-center justify-center p-0 backdrop-blur-sm sm:p-3 ${backdropClassName}`} onClick={(event) => { if (event.target === event.currentTarget) onClose(); }} role="dialog">
