...
function Toggle({ checked, description, disabled = false, disabledReason, label, onChange }: { checked: boolean; description: string; disabled?: boolean; disabledReason?: string; label: string; onChange: (checked: boolean) => void }) {
  return <label className={`group flex items-start justify-between gap-4 rounded-2xl border p-3.5 transition ${disabled ? "border-white/[0.05] bg-white/[0.025] text-white/35" : "border-white/[0.08] bg-[var(--coinche-settings-surface)] text-stone-100 shadow-inner hover:border-emerald-300/25 hover:bg-white/[0.07]"}`}><span><span className="block text-sm font-bold text-[#f4ead0] group-has-[:disabled]:text-white/35">{label}</span><span className="mt-1 block text-xs leading-relaxed text-white/55 group-has-[:disabled]:text-white/30">{disabled && disabledReason ? disabledReason : description}</span></span><span className="relative mt-0.5 shrink-0"><input aria-label={label} checked={checked} className="peer absolute inset-0 z-10 h-full w-full cursor-pointer opacity-0" disabled={disabled} onChange={(event) => onChange(event.target.checked)} type="checkbox" /><span aria-hidden="true" className="pointer-events-none block h-6 w-11 rounded-full bg-white/15 shadow-inner transition peer-checked:bg-emerald-600 peer-focus-visible:outline peer-focus-visible:outline-3 peer-focus-visible:outline-offset-2 peer-focus-visible:outline-emerald-300 peer-disabled:opacity-40 after:absolute after:left-0.5 after:top-0.5 after:h-5 after:w-5 after:rounded-full after:bg-[#fffdf7] after:shadow after:transition-transform after:content-[''] peer-checked:after:translate-x-5" /></span></label>;
}

function Field({ children, description, label }: { children: React.ReactNode; description?: string; label: string }) {
  return <label className="block rounded-2xl border border-white/[0.08] bg-[var(--coinche-settings-surface)] p-3.5 text-sm font-bold text-[#f4ead0] shadow-inner">{label}{description ? <span className="mt-1 block text-xs font-normal leading-relaxed text-white/55">{description}</span> : null}{children}</label>;
}
...
function TimingField({ label, max, step, value, onChange }: { label: string; max: number; step: number; value: number; onChange: (value: number) => void }) {
  return <Field label={label}><div className="mt-2 flex items-center gap-3"><input aria-label={label} className="min-w-0 flex-1 accent-emerald-500" max={max} min={0} onChange={(event) => onChange(Number(event.target.value))} step={step} type="range" value={value} /><output className="w-16 text-right font-mono text-xs text-emerald-300">{formatPreferenceDuration(value)}</output></div></Field>;
}
...
export function PlayerSettingsPanel({ context = SOLO_SETTINGS_CONTEXT }: { context?: PlayerSettingsContext }) {
  ...
  return <div className="coinche-settings-panel flex h-full min-h-0 flex-1 flex-col bg-[var(--coinche-settings-canvas)] text-stone-100">
    <div className="shrink-0 border-b border-white/10 bg-[var(--coinche-settings-header)] px-4 py-2.5 sm:px-6"><input aria-label="Rechercher un paramètre" className="w-full rounded-xl border border-white/15 bg-[var(--coinche-settings-field)] px-3 py-2.5 text-sm text-stone-100 shadow-inner outline-none placeholder:text-white/40 focus:border-emerald-300/50 focus:ring-2 focus:ring-emerald-300/15" onChange={(event) => setQuery(event.target.value)} placeholder="Rechercher un paramètre" type="search" value={query} /></div>
    <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden md:grid md:grid-cols-[210px_minmax(0,1fr)] md:grid-rows-[minmax(0,1fr)]">
      <nav aria-label="Sections des paramètres" className="flex shrink-0 overflow-x-auto border-b border-white/10 bg-[var(--coinche-settings-sidebar)] p-2 md:flex-col md:overflow-hidden md:border-b-0 md:border-r md:p-4">
        <div className="flex shrink-0 gap-1.5 md:min-h-0 md:flex-1 md:flex-col md:overflow-y-auto md:overscroll-contain">
          {visibleSections.map((section) => <button aria-current={selected === section.id ? "page" : undefined} className={`whitespace-nowrap rounded-xl border px-3 py-2.5 text-left text-sm font-bold transition ${selected === section.id ? "border-[#ead8a6]/30 bg-[#ead8a6] text-[#10251b] shadow" : "border-transparent bg-transparent text-white/55 hover:border-white/10 hover:bg-white/[0.06] hover:text-white"}`} key={section.id} onClick={() => setActive(section.id)} type="button">{section.label.toLocaleUpperCase("fr")}</button>)}
        </div>
        <div className="ml-2 flex shrink-0 items-center gap-1.5 border-l border-white/10 pl-2 md:ml-0 md:mt-3 md:block md:border-l-0 md:border-t md:pl-0 md:pt-3">
          {confirmReset ? <div className="flex gap-1.5 md:grid"><button className="rounded-lg border border-red-300/20 bg-red-950/35 px-2.5 py-2 text-xs font-bold text-red-100 transition hover:bg-red-900/45" onClick={() => { reset(); setConfirmReset(false); }} type="button">Confirmer</button><button className="rounded-lg border border-white/10 bg-white/[0.05] px-2.5 py-2 text-xs font-bold text-white/70 transition hover:bg-white/10 hover:text-white" onClick={() => setConfirmReset(false)} type="button">Annuler</button></div> : <button className="whitespace-nowrap rounded-lg px-2.5 py-2 text-left text-xs font-semibold text-white/55 transition hover:bg-white/[0.06] hover:text-red-200" onClick={() => setConfirmReset(true)} type="button">Réinitialiser mes paramètres</button>}
        </div>
      </nav>
      <div className="min-h-0 min-w-0 flex-1 touch-pan-y overflow-y-auto overscroll-contain bg-[radial-gradient(circle_at_top_right,rgb(31_101_69_/_16%),transparent_45%)] p-4 [scrollbar-gutter:stable] sm:p-6">
        {!selected ? <p className="rounded-xl border border-white/10 bg-white/[0.05] p-4 text-sm text-white/70">Aucun paramètre ne correspond à « {query} ».</p> : null}
...
function GameSettings({ context, preferences, setGameSpeed, speedLabels, timing, update }: { context: PlayerSettingsContext; preferences: PlayerPreferences; setGameSpeed: (speed: PresetGameSpeed) => void; speedLabels: Record<PresetGameSpeed | "custom", string>; timing: (key: "botDelayMs" | "trickDisplayMs" | "biddingDelayMs", value: number) => void; update: Update }) {
  return <section aria-labelledby="settings-game" className="space-y-3"><h3 className="text-lg font-bold text-[#f4ead0]" id="settings-game">Jeu</h3>{context.mode === "solo" ? <><Field description="Règle uniquement le rythme visuel. Le moteur calcule toujours immédiatement." label="Vitesse de jeu"><select aria-label="Vitesse de jeu" className="mt-2 w-full rounded-lg border border-white/15 px-3 py-2" onChange={(event) => { if (event.target.value !== "custom") setGameSpeed(event.target.value as PresetGameSpeed); }} value={preferences.gameplay.gameSpeed}>{Object.entries(speedLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></Field><Toggle checked={preferences.gameplay.autoCollectTricks} description={`Ramasse après ${formatPreferenceDuration(preferences.gameplay.trickDisplayMs)}. Sinon, un bouton indique qui a gagné.`} label="Ramasser les plis automatiquement" onChange={(v) => update("gameplay", { autoCollectTricks: v })} /></> : <TablePacingSettings context={context} speedLabels={speedLabels} />}<div className="grid gap-2 sm:grid-cols-2"><Toggle checked={preferences.gameplay.confirmCoinche} description="Demander avant de contrer." label="Confirmer la Coinche" onChange={(v) => update("gameplay", { confirmCoinche: v })} /><Toggle checked={preferences.gameplay.confirmSurcoinche} description="Demander avant de surcontrer." label="Confirmer la Surcoinche" onChange={(v) => update("gameplay", { confirmSurcoinche: v })} /><Toggle checked={preferences.gameplay.confirmCapot} description="Demander avant d'annoncer un Capot." label="Confirmer le Capot" onChange={(v) => update("gameplay", { confirmCapot: v })} /><Toggle checked={preferences.gameplay.confirmGenerale} description="Demander avant d'annoncer une Générale." label="Confirmer la Générale" onChange={(v) => update("gameplay", { confirmGenerale: v })} /></div>{context.mode === "solo" ? <details className="rounded-xl border border-white/[0.08] bg-white/[0.04] p-3"><summary className="cursor-pointer font-bold text-[#f4ead0]">Réglages avancés <span className="font-normal text-white/45">· délais précis</span></summary><div className="mt-3 grid gap-2"><TimingField label="Temps de réflexion visuel des bots" max={2000} onChange={(v) => timing("botDelayMs", v)} step={50} value={preferences.gameplay.botDelayMs} /><TimingField label="Durée d'affichage d'un pli" max={3000} onChange={(v) => timing("trickDisplayMs", v)} step={50} value={preferences.gameplay.trickDisplayMs} /><TimingField label="Délai entre enchères" max={1500} onChange={(v) => timing("biddingDelayMs", v)} step={50} value={preferences.gameplay.biddingDelayMs} /></div></details> : null}</section>;
}
...
function TablePacingSettings({ context, speedLabels }: { context: Extract<PlayerSettingsContext, { mode: "multiplayer" }>; speedLabels: Record<PresetGameSpeed | "custom", string> }) {
  ...
  if (!context.isHost || !context.onTablePreferencesChange) {
    return <div className="rounded-2xl border border-white/[0.08] bg-[var(--coinche-settings-surface)] p-3.5 text-sm text-stone-100 shadow-inner"><strong className="text-[#f4ead0]">Rythme de la table : {speedLabels[settings.gameSpeed]}</strong><span className="mt-1 block text-xs leading-relaxed text-white/55">{context.tablePreferences ? "Défini par l’hôte et appliqué à tous les joueurs." : "Il sera défini par l’hôte une fois dans la table."}</span></div>;
  }
  const changed = JSON.stringify(draft) !== JSON.stringify(settings);
  return <div className="space-y-3 rounded-2xl border border-white/[0.08] bg-[var(--coinche-settings-surface)] p-3.5 text-stone-100 shadow-inner"><div><h4 className="font-bold text-[#f4ead0]">Rythme de la table</h4><p className="mt-1 text-xs leading-relaxed text-white/55">Ces réglages sont partagés avec tous les joueurs.</p></div><Field description="Règle la présentation commune des plis." label="Vitesse de jeu"><select aria-label="Vitesse de jeu de la table" className="mt-2 w-full rounded-lg border border-white/15 px-3 py-2" onChange={(event) => { if (event.target.value !== "custom") setDraft((current) => withMultiplayerTableSpeed(current, event.target.value as PresetGameSpeed)); }} value={draft.gameSpeed}>{Object.entries(speedLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></Field><Toggle checked={draft.autoCollectTricks} description={`Ramasse après ${formatPreferenceDuration(draft.trickDisplayMs)}. Sinon, chaque joueur voit le bouton de ramassage.`} label="Ramassage automatique des plis" onChange={(autoCollectTricks) => setDraft((current) => ({ ...current, autoCollectTricks }))} /><details className="rounded-xl border border-white/[0.08] bg-black/10 p-3"><summary className="cursor-pointer font-bold text-[#f4ead0]">Réglages avancés <span className="font-normal text-white/45">· délai réellement appliqué</span></summary><div className="mt-3"><TimingField label="Durée d'affichage d'un pli" max={3000} onChange={(value) => setDraft((current) => withMultiplayerTableTrickDisplay(current, value))} step={50} value={draft.trickDisplayMs} /></div></details><button className="w-full rounded-lg bg-emerald-700 px-3 py-2 text-sm font-bold text-white transition hover:bg-emerald-600 focus-visible:outline focus-visible:outline-3 focus-visible:outline-offset-2 focus-visible:outline-emerald-300 disabled:cursor-not-allowed disabled:opacity-50" disabled={!changed || context.isSavingTablePreferences} onClick={() => context.onTablePreferencesChange?.(draft)} type="button">{context.isSavingTablePreferences ? "Application…" : "Appliquer à la table"}</button></div>;
}
...
function HelpSettings({ preferences, update }: { preferences: PlayerPreferences; update: Update }) {
  ...
  return <section aria-labelledby="settings-help" className="space-y-2"><h3 className="text-lg font-bold text-[#f4ead0]" id="settings-help">Aides</h3>{rows.map(([key,label,description]) => <Toggle checked={preferences.assistance[key]} description={description} key={key} label={label} onChange={(v) => update("assistance", { [key]: v })} />)}</section>;
}

function CardSettings({ preferences, update }: { preferences: PlayerPreferences; update: Update }) {
  return <section aria-labelledby="settings-cards" className="space-y-3"><h3 className="text-lg font-bold text-[#f4ead0]" id="settings-cards">Cartes</h3><Toggle checked={preferences.cards.autoSortHand} description="Réordonne uniquement l'affichage, jamais la main du moteur." label="Ranger automatiquement les cartes" onChange={(v) => update("cards", { autoSortHand: v })} /><Field label="Mode de tri"><select aria-label="Mode de tri des cartes" className="mt-2 w-full rounded-lg border px-3 py-2" disabled={!preferences.cards.autoSortHand} onChange={(event) => update("cards", { sortMode: event.target.value as PlayerPreferences["cards"]["sortMode"] })} value={preferences.cards.sortMode}><option value="suit-rank">Par couleur puis valeur</option><option value="rank-suit">Par valeur puis couleur</option></select></Field><fieldset className="rounded-xl border border-white/[0.08] bg-[var(--coinche-settings-surface)] p-3 text-stone-100 shadow-inner"><legend className="px-1 text-sm font-semibold text-[#f4ead0]">Ordre des couleurs</legend><p className="mb-2 text-center text-xl" aria-live="polite">{preferences.cards.suitOrder.map((suit) => SUIT_SYMBOLS[suit]).join(" → ")}</p><div className="grid gap-2 sm:grid-cols-2">{preferences.cards.suitOrder.map((suit, index) => <div className="flex items-center justify-between rounded-lg border border-white/[0.08] bg-black/10 px-3 py-2" key={suit}><span className="font-semibold">{SUIT_SYMBOLS[suit]} {SUIT_LABELS[suit]}</span><span className="flex gap-1"><button aria-label={`Monter ${SUIT_LABELS[suit]}`} className="rounded border border-white/15 bg-white/[0.05] px-2 py-1 transition hover:bg-white/10 disabled:opacity-30" disabled={index === 0} onClick={() => update("cards", { suitOrder: moveSuit(preferences.cards.suitOrder, suit, -1) })} type="button">↑</button><button aria-label={`Descendre ${SUIT_LABELS[suit]}`} className="rounded border border-white/15 bg-white/[0.05] px-2 py-1 transition hover:bg-white/10 disabled:opacity-30" disabled={index === 3} onClick={() => update("cards", { suitOrder: moveSuit(preferences.cards.suitOrder, suit, 1) })} type="button">↓</button></span></div>)}</div></fieldset><div className="grid gap-2 sm:grid-cols-2"><Field label="Taille des cartes"><select aria-label="Taille des cartes" className="mt-2 w-full rounded-lg border px-3 py-2" onChange={(event) => update("cards", { cardSize: event.target.value as PlayerPreferences["cards"]["cardSize"] })} value={preferences.cards.cardSize}><option value="small">Petite</option><option value="medium">Normale</option><option value="large">Grande</option></select></Field><Field label="Style des cartes"><select aria-label="Style des cartes" className="mt-2 w-full rounded-lg border px-3 py-2" onChange={(event) => update("cards", { cardStyle: event.target.value as PlayerPreferences["cards"]["cardStyle"] })} value={preferences.cards.cardStyle}><option value="classic">Classique</option><option value="modern">Moderne</option></select></Field></div></section>;
}
