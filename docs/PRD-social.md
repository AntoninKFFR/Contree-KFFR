...
| `/profile`, `AuthStatus`, `AppDrawerNav` connaissent déjà la session et le pseudo ([profil](../app/profile/page.tsx), [statut](../components/AuthStatus.tsx), [drawer](../components/AppDrawerNav.tsx)) | Ajouter `/friends` et un lien conditionnel au drawer ; réutiliser l'auth existante. L'email affiché sur **son propre** profil ne doit jamais apparaître dans une réponse sociale. |
...
| `send_game_invitation(p_room_id uuid, p_invitee_id uuid)` | Crée pending/retourne l'invitation existante, expiration 30 min. | Verrouille la room, revérifie lobby + siège empty, expéditeur humain assis, amitié active, invité non assis, aucune pending pour room/invité ; quotas **20 invitations/jour** et anti-rafale par paire/room. |
...
| Deux invitations vers même room/invité | Une pending grâce à l'index unique, même si les invitants diffèrent ; nouvel envoi retourne l'existante sans spam. |
...
5. **E2E réel à quatre comptes dédiés.** A demande B → B accepte → chacun voit l'amitié → A crée une room → A invite B → B reçoit l'invitation → B clique Rejoindre → B prend un siège via `join-seat` → invitation acceptée. C et D ne lisent ni demande ni invitation ; aucune donnée privée supplémentaire n'est exposée. Ajouter les cas négatifs RLS, refus/annulation, room devenue pleine, course de sièges et reconnexion/retard Realtime. Ne jamais enregistrer de mots de passe dans Git ni traces Playwright.
